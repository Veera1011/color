...
from ..models import Academics, PersonalInformation,Scholarship,RejoinStudent,FinalYearProject,FinalYearProjectthesis
...
class FPView(View):
    ...
    def get_context(self, user):
        s_data = FinalYearProject.objects.filter(roll_number__roll_number=user.username).first()
        return {
            's_data': s_data,
            'roll_number': user.username
        }
    ...
    def get(self, request):
        ...
        try:
            ...
            context = self.get_context(user)
            if request.headers.get('HX-Request'):
                return render(request, 'fp/fp_form.html', context)
            return render(request, 'fp/fp_details.html', context)
    ...
    def post(self, request):
        ...
        try:
            ...
            s_data = {
                ...
                'problem_statement': request.FILES.get('problem_statement'),  # File upload
            }
              
            final_project, created = FinalYearProject.objects.update_or_create(
                roll_number=personal_info,
                defaults=s_data
            )

            context = self.get_context(user)
            context['message'] = 'Project information saved successfully!'
            return render(request, 'fp/fp_form.html', context)
